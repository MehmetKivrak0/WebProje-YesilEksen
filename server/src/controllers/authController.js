...
const register = async (req, res) => {
    ...
    try {
        ...
        if (rol === 'ciftci') {
            ...
            // Firma başvurusu oluştur (normalizasyon: başvuru ve kayıtlı firma ayrı)
            ...
            const basvuranAdi = `${firstName} ${lastName}`;
            
            if (process.env.NODE_ENV === 'development') {
                console.log('🔍 Firma başvurusu oluşturuluyor:', {
                    kullanici_id: user.id,
                    firma_adi: firmaName,
                    basvuran_adi: basvuranAdi,
                    vergi_no: vergiNo,
            ...
            // Önce firma_basvurulari tablosuna başvuru ekle
            // Not: firma_id henüz yok, onaylandıktan sonra firmalar tablosuna geçecek
            const basvuruResult = await client.query(
                `INSERT INTO firma_basvurulari 
                (kullanici_id, firma_adi, basvuran_adi, vergi_no, telefon, eposta, adres, durum)
                VALUES ($1, $2, $3, $4, $5, $6, $7, 'beklemede')
                RETURNING id`,
                [user.id, firmaName, basvuranAdi, vergiNo, user.telefon, user.eposta, firmaAdres]
            );
            const basvuruId = basvuruResult.rows[0].id;

            if (process.env.NODE_ENV === 'development') {
                console.log('✅ Firma başvurusu oluşturuldu:', {
                    basvuru_id: basvuruId,
                    firma_adi: firmaName,
                    durum: 'beklemede'
                });
            ...
            for (const [fileKey, belgeKod] of Object.entries(firmaBelgeTypes)) {
                ...
                if (fileArray && fileArray.length > 0) {
                    ...
                    await client.query(
                        `INSERT INTO belgeler 
                        (kullanici_id, basvuru_id, basvuru_tipi, belge_turu_id, ad, dosya_yolu, dosya_boyutu, dosya_tipi, durum, zorunlu)
                        VALUES ($1, $2, 'firma_basvurusu', $3, $4, $5, $6, $7, 'beklemede', $8)`,
                        [user.id, basvuruId, belgeTuruId, file.originalname, filePath, fileSize, fileExt, true]
                    );
                }
            }
